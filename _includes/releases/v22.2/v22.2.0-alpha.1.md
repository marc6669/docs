...
- Added [`DROP EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/drop-external-connection.html) to drop a previously created external connection object. [#84751][#84751]
...
- A Google Cloud Storage KMS can be represented as an [external connection](../{{site.versions["dev"]}}/create-external-connection.html) object, which can be used during [`BACKUP`](../{{site.versions["dev"]}}/backup.html) or [`RESTORE`](../{{site.versions["dev"]}}/restore.html) using the `external` URI. [#85075][#85075]
...
- The [`CREATE EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/create-external-connection.html) statement can be used to represent an underlying [`userfile`](../{{site.versions["dev"]}}/use-userfile-for-bulk-operations.html) resource. [#86006][#86006]
...
- [Session setting](../{{site.versions["dev"]}}/set-vars.html) `optimizer_use_not_visible_indexes` can be used to disable not visible index features. When this setting is enabled, the optimizer treats not visible indexes as if they were visible and can choose to use them for query planning. By default, this setting is disabled. [#86033][#86033]
...
- The [`CREATE EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/create-external-connection.html) statement can be used to represent an underlying [Google Cloud Storage resource](../{{site.versions["dev"]}}/use-cloud-storage-for-bulk-operations.html). [#85964][#85964]
...
- The [`CREATE EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/create-external-connection.html) statement can be used to represent an [Azure Storage URI](../{{site.versions["dev"]}}/use-cloud-storage-for-bulk-operations.html). [#86257][#86257]
- Added the [`SHOW CREATE EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/show-create-external-connection.html) and `SHOW CREATE ALL EXTERNAL CONNECTIONS` statements, which display the connection name and the unredacted query used to create the external connection. Currently, this can only be run by users of the `admin` role. [#86161][#86161]
...
- The [`CREATE EXTERNAL CONNECTION`](../{{site.versions["dev"]}}/create-external-connection.html) statement can be used to represent an `aws-kms` scheme that represents an Amazon S3 KMS resource. [#86402][#86402]
...
- Added the `rebalancing.writebytespersecond` and `rebalancing.readbytespersecond` [time series metrics](../{{site.versions["dev"]}}/monitoring-and-alerting.html). These metrics reflect the average number of bytes written and read across all replicas per store, over the last 30 minutes. [#80245][#80245]
...
- Added index created time as an option on the DB Console [Databases page](../{{site.versions["dev"]}}/ui-databases-page.html#tables-view). [#78283][#78283]
- Users can now see actively running queries and transactions in the [SQL Activity page](../{{site.versions["dev"]}}/ui-overview.html#sql-activity). The transactions and statements tabs in SQL activity now have a menu to show either active or historical transactions and statements data. [#76753][#76753]
...
- The [Active Statements](../{{site.versions["dev"]}}/ui-overview.html#sql-activity) and Active Transactions pages now have a single filter option for internal apps. These pages no longer display internal statements and transactions by default. [#83014][#83014]
...
- Changed the Plans table within the **Explain Plan** tab of the [Statement Details](../{{site.versions["dev"]}}/ui-statements-page.html) page to use a plan gist instead of the plan ID. Also added the plan gist as the first line on the Explain Plan display. [#86653][#86653]
...
- Fixed the `has_sequence_privilege()` [built-in function](../{{site.versions["dev"]}}/functions-and-operators.html) checking on the `USAGE` privilege. [#82458][#82458]
- Fixed a bug where [backups](../{{site.versions["dev"]}}/take-full-and-incremental-backups.html) in the base directory of a Google Storage bucket would not be discovered by [`SHOW BACKUPS`](../{{site.versions["dev"]}}/show-backup.html). These backups will now appear correctly. [#80182][#80182]
- Fixed an [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) bug that prevented expressions of the form `(NULL::STRING[] <@ ARRAY['x'])` from being folded to `NULL`. [#77995][#77995]
...
- [`SHOW EXPERIMENTAL_FINGERPRINTS FROM TABLE`](../{{site.versions["dev"]}}/experimental-features.html#show-table-fingerprints) now works on tables with partial indexes. [#80539][#80539]
...
- The hex encoding for [`BYTEA`](../{{site.versions["dev"]}}/bytes.html) values now works properly when used in [`COPY FROM ... CSV`](../{{site.versions["dev"]}}/copy-from.html) statements. [#81120][#81120]
...
- Fixed a bug where [sequences](../{{site.versions["dev"]}}/create-sequence.html) could return values that are out-of-bounds in some cases. [#81123][#81123]
...
- The [`DateStyle`](../{{site.versions["dev"]}}/show-vars.html#supported-variables) session setting is no longer ignored using the CLI when set in the `options` URL parameter. [#82101][#82101]
...
- [`CREATE TABLE AS`](../{{site.versions["dev"]}}/create-table-as.html) in explicit transactions would fail with an error if the size of the source table exceeded the Raft command size limit. [#82951][#82951]
- Range lease transfers are no longer permitted to follower replicas that may require a [Raft snapshot](../{{site.versions["dev"]}}/architecture/replication-layer.html#snapshots). This ensures that lease transfers are never delayed behind snapshots, which could previously create range unavailability until the snapshot completed. Lease transfers are now only allowed when the outgoing leaseholder can guarantee that the incoming leaseholder does not need a snapshot. [#82758][#82758]
- Fixed a bug where creating a unique, expression index on a [`REGIONAL BY TABLE`](../{{site.versions["dev"]}}/multiregion-overview.html) could result in an error. [#83125][#83125]
...
- Previously, the `querySummary` metadata field in the [`crdb_internal.statement_statistics`](../{{site.versions["dev"]}}/crdb-internal.html) table was inconsistent with the query metadata field for executed prepared statements. These fields are now consistent for prepared statements. [#83673][#83673]
- Fixed a bug where [`BACKUP`](../{{site.versions["dev"]}}/backup.html) may be missing data when the cluster was configured with very low values for `kv.bulk_sst.max_allowed_overage` and `kv.bulk_sst.target_size` cluster settings. [#83102][#83102]
- Fixed an issue where some [exports](../{{site.versions["dev"]}}/export.html) would receive `"unexpected closure of consumer"` rather than the actual error the export encountered. [#77938][#77938]
- Fixed a bug causing a graceful node shutdown to stall forever. [#83824][#83824]
- The `PASSWORD` option of the [`CREATE`](../{{site.versions["dev"]}}/create-role.html)/[`ALTER ROLE`](../{{site.versions["dev"]}}/alter-role.html) commands now requires the password to be surrounded with single quotes. This fixes confusion that could arise when a mixed-case string is used, since previously that would cause the password to be normalized to lowercase. [#83924][#83924]
...
- Fixed a bug that led to the `querySummary` field in the [`crdb_internal.statements_statistics`](../{{site.versions["dev"]}}/crdb-internal.html) metadata column being empty. [#84170][#84170]
...
- Fixed a bug where, in an [`ALTER PRIMARY KEY`](../{{site.versions["dev"]}}/alter-primary-key.html) statement, if the new primary key columns is a subset of the previous primary key columns, CockroachDB would not rewrite existing secondary indexes, and hence those secondary indexes continue to have some of the previous primary key columns in their `suffixColumns`. But the user might, reasonably think those columns are not used anymore and proceed to drop them. The bug then caused those dependent secondary indexes to be dropped, unexpectedly for the user. [#84303][#84303]
- Fixed a bug where the [CLI `cockroach` commands](../{{site.versions["dev"]}}/cockroach-commands.html) could produce spurious `"latency jump"` warnings when connecting to a remote server. This bug had been introduced in CockroachDB v21.2. [#84031][#84031]
...
- When a CockroachDB [node is being drained](../{{site.versions["dev"]}}/node-shutdown.html#drain-a-node-manually), all queries that are still running on that node are now forcefully canceled after waiting the `server.shutdown.query_wait` period. [#82752][#82752]
- The SQL [Unix socket](../{{site.versions["dev"]}}/connection-parameters.html#example-uri-for-a-unix-domain-socket), when requested, now contains a port number compatible with the connection URL when `--listen-addr` is configured to auto-allocate a port number. This bug had existed since CockroachDB v1.0. [#84910][#84910]
- Previously, if a [Unix socket](../{{site.versions["dev"]}}/connection-parameters.html#example-uri-for-a-unix-domain-socket) was requested but it already existed on disk, CockroachDB would exit with an error even if the original owner process was not running. This limitation would, for example, prevent reuse of a Unix socket after an abnormal shutdown. It had been present since CockroachDB v1.0. This is now fixed. [#84910][#84910]
...
- Fixed a bug where, in a stage of validation operations in the [declarative schema changer](../{{site.versions["dev"]}}/online-schema-changes.html#declarative-schema-changer), only the first validation operation is properly handled and the rest are skipped. [#85781][#85781]
- Fixed a bug internal to drawing dependency graph of a DDL statement under the [declarative schema changer](../{{site.versions["dev"]}}/online-schema-changes.html#declarative-schema-changer). [#85773][#85773]
- Fixed a rare bug where errors could occur related to the use of arrays of type [`ENUM`](../{{site.versions["dev"]}}/enum.html). [#85940][#85940]
...
- Users that create an [external connection](../{{site.versions["dev"]}}/create-external-connection.html) are now granted `ALL` privileges on the object. [#86336][#86336]
...
- Fixed a longstanding bug that could cause the [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) to produce an incorrect plan when aggregate functions `st_makeline` or `st_extent` were called with invalid-type and empty inputs respectively. [#86722][#86722]
- Fixed a crash that could occur when formatting queries that have placeholder `BitArray` arguments. [#86607][#86607]
- Fixed a crash/panic that could occur if placeholder arguments were used with the `with_min_timestamp()` or `with_max_staleness()` [functions](../{{site.versions["dev"]}}/functions-and-operators.html). [#86605][#86605]
...
- Rollback of [materialized view](../{{site.versions["dev"]}}/views.html#materialized-views) creation left references inside dependent objects. This fix adds clean up to the back/forward references for materialized views. [#82087][#82087]
...
- The [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) now explores more efficient query plans when indexing [computed columns](../{{site.versions["dev"]}}/computed-columns.html) and expressions that have `IS NULL` expressions. [#83619][#83619]
- The [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) can now return the results of a join in sorted order in more cases. This can allow the optimizer to avoid expensive sorts that need to buffer all input rows. [#84689][#84689]
- The [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) is now less likely to take an excessive amount of time to plan queries with many joins. [#85100][#85100]
- The [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) can detect contradictory filters in more cases, leading to more efficient query plans. [#85351][#85351]
- The row-level TTL job has been modified to distribute work using [DistSQL](../{{site.versions["dev"]}}/architecture/sql-layer.html#distsql). This usually results in the leaseholder nodes managing deletes of the spans they own. [#84728][#84728]
- The execution engine can now short-circuit execution of [lookup joins](../{{site.versions["dev"]}}/joins.html) in more cases, which can decrease latency for queries with limits. [#85731][#85731]
- `ILIKE` and `NOT ILIKE` filters can now be evaluated more efficiently in some cases. [#85695][#85695]
- [MVCC garbage collection](../{{site.versions["dev"]}}/architecture/storage-layer.html#garbage-collection) should now be much less disruptive to foreground traffic than previously. [#83213][#83213]
- The execution engine can now perform [lookup joins](../{{site.versions["dev"]}}/joins.html) in more cases. This can significantly improve join performance when there is a large table with an index that conforms to the join `ON` conditions, as well as allow joins to halt early in the presence of a limit. [#85597][#85597]
- Point deletes in SQL are now more efficient during concurrent workloads. [#63416][#63416]
- Enabled table statistics forecasts, which predict future statistics based on historical collected statistics. Forecasts help the [optimizer](../{{site.versions["dev"]}}/cost-based-optimizer.html) produce better plans for queries that read data modified after the latest statistics collection. CockroachDB only uses the forecasts that fit the historical collected statistics very well, meaning it has high confidence in their accuracy. Forecasts can be viewed using `SHOW STATISTICS FOR TABLE ... WITH FORECAST`. [#86078][#86078]
- Optimized the execution of [`COPY FROM`](../{{site.versions["dev"]}}/copy-from.html). [#83840][#83840]
...
- [SQL statements](../{{site.versions["dev"]}}/sql-statements.html) that cause events to be logged to `system.eventlog` are now able to complete faster. [#86174][#86174]
...
- The default [L0 sub-level enforcement](../{{site.versions["dev"]}}/architecture/storage-layer.html#lsm-levels) for rebalancing and allocation decisions is now set to `block_rebalance_to`. This has the effect of stopping rebalancing to stores that have high read amplification. [#79794][#79794]
